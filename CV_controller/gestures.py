...
class GestureClassifier:
    """
    Classificatore di gesti basato su Machine Learning.
    Riconosce il gesto dell'utente analizzando le posizioni (landmark) rilevate da MediaPipe.
    """
    ...
    def _load_model(self):
        """Carica il modello (.pkl) e il corrispondente encoder dei tag."""
    ...
    def predict_gesture(self, landmarks):
        """
        Riconosce il gesto corrente estraendo e classificando le caratteristiche dei landmark.
        Ritorna una tupla: (nome_gesto, percentuale_confidenza).
        """
        if self.model is None or self.encoder is None:
            return "unknown", 0.0

        # Estrazione delle feature (x, y, z, v per ciascuno dei 33 landmark)
        features = []
        for lm in landmarks:
