...
class Player(object):
    ...
    def player_physics(self, core):
        ...
        # Rimozione dell'errore computazionale
        ...
        if not self.on_ground:
            # Movimento verso l'alto, il tasto è premuto
            if (self.y_vel < 0 and core.keyU):
                self.y_vel += GRAVITY
                
            # Movimento verso l'alto, il tasto non è premuto - salto basso
        ...
        # Il parametro on_ground non sarebbe stabile senza questo pezzo di codice
        ...
        # Controllo collisione con la bandiera di fine livello
    ...
    def update_image(self, core):
        ...
        if self.powerLVL in (0, 1, 2):
            ...
            # Il giocatore ha deciso di muoversi nell'altra direzione, ma non si è ancora fermato
    ...
    def flag_animation_move(self, core, walk_to_castle):
        if walk_to_castle:
            ...
            # on_ground non funziona correttamente senza questo pezzo di codice
            x = self.rect.x // 32
            y = self.rect.y // 32
            if self.powerLVL > 0:
                y += 1
