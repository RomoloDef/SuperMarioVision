...
class Koopa(Entity):
    ...
    def change_state(self, core):
        ...
        # 0 to 1 - Se il koopa è colpito da sopra, si nasconde nella sua conchiglia.
        if self.rect.h == 46:
            self.x_vel = 0
            self.rect.h = 32
            self.rect.y += 14
        ...
        # 1 to 2 - Se il koopa è colpito mentre è nascosto, inizia a muoversi velocemente nella conchiglia.
        elif self.state == 2:
            core.get_map().get_player().add_score(100)
            core.get_map().spawn_score_text(self.rect.x + 16, self.rect.y, score=100)
        ...
        # 2 to 3 - Se il koopa è colpito mentre è veloce, muore.
        elif self.state == 3:
